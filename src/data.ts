...
export const IMAGES: Record<string, string> = {
  player,
  e_furry: eFurry, e_feline: eFeline, e_speedbug: eSpeedbug, e_ladybug: eLadybug,
  e_helmetbee: eHelmetbee, e_bomberkitty: eBomberkitty, e_razorclaw: eRazorclaw,
  e_hammerhead: eHammerhead, e_boss: eBoss,
  sky, clouds1, clouds2, appicon,
  gem_green: gemGreen, gem_blue: gemBlue, gem_gold: gemGold, coin, meat, magnet, chest,
  bullet, bullet_long: bulletLong,
  w_croissant: wCroissant, w_yarnball: wYarnball, w_propeller: wPropeller,
  w_fish: wFish, w_shield: wShield,
  i_multicanon: iMulticanon, i_croissant: iCroissant, i_lightning: iLightning,
  i_shield: iShield, i_propeller: iPropeller, i_yarnball: iYarnball,
  i_razorfin: iRazorfin, i_warmachine: iWarmachine,
  i_attack: iAttack, i_speed: iSpeed, i_health: iHealth, i_armor: iArmor,
  i_magnet: iMagnet, i_xp: iXp, i_cooldown: iCooldown, i_bulletspeed: iBulletspeed,
  hud_time: hudTime, hud_kills: hudKills, hud_wave: hudWave
};
...
export interface EnemyDef {
  key: string; hp: number; speed: number; damage: number; scale: number;
  radius: number; gem: number; boss?: boolean;
}
...
export interface Wave {
  start: number; end: number; pool: string[]; interval: number; burst: number; cap: number;
}
...
export const SKILLS: SkillDef[] = [
  ...
  { id: 'lightning',  title: 'Shockwave Strike',  desc: "That's one flashy attack.",                  icon: 'i_lightning',  kind: 'weapon',  max: 5 },
  { id: 'shield',     title: 'Protective Shield', desc: 'Hold these pesky kitties back!',             icon: 'i_shield',     kind: 'weapon',  max: 5 },
  { id: 'propeller',  title: 'Fiesty Propeller',  desc: "Shouldn't these be attached to the plane?",  icon: 'i_propeller',  kind: 'weapon',  max: 5 },
  ...
  { id: 'warmachine', title: 'War Machine',       desc: 'Nothing beats good old guns!',               icon: 'i_warmachine', kind: 'weapon',  max: 5 },
  ...
  { id: 'bulletspeed', title: 'Cheese Chaser',    desc: 'Bullet Flight Speed +10%',        icon: 'i_bulletspeed', kind: 'passive', max: 5 }
];
